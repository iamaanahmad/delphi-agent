...
test("creates a dry-run plan when evidence and quote pass", async () => {
  const decision = await evaluateMarket(new ReplayGateway([market]), market, 0, [evidence], policy);
...
test("stops on stale evidence", async () => {
  ...
  const decision = await evaluateMarket(new ReplayGateway([market]), market, 0, [stale], policy);
