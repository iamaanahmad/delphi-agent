...
- Mapping: `items.0.views gt 2250`; source timestamp `items.0.timestamp` in `yyyyMMddHH` UTC format.
...
- Mapping: select `data` records inside the exact UTC window, parse `v`, and apply `max gt 5.18`; each record carries its source timestamp at `t`.
...
- Mapping: require `match_information.match_status eq finalWhistle`, then compare `home_team_goals gt away_team_goals`; the source timestamp is `match_information.kickoff_time`.
- Market page: https://competition.delphi.fyi/markets/0xbf1ce7c9d751b92bfac4acefe0e87d82b1d30163

## Safety boundary

The reviewed pack identifies settlement facts; it does not claim a profitable timing edge. Source and global freshness checks both remain active, and `.env.example` keeps `ALLOW_LIVE_TRADING=false` and `SETTLEMENT_EDGE_EXECUTE=false`.
