...
const evidence: EvidenceSignal = { id: "primary", source: "Primary API", sourceUrl: "https://example.test", eventTime: "2020-01-01T00:00:00.000Z", freshnessTime: now, freshnessType: "retrieval", probability: 0.995, confidence: 0.98, detail: "threshold crossed" };
...
test("stops when the freshness signal is stale", async () => {
  const directory = await mkdtemp(join(tmpdir(), "settlement-edge-engine-"));
  const stale = { ...evidence, freshnessTime: "2020-01-01T00:00:00.000Z" };
